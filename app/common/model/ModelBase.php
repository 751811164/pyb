...
class ModelBase extends Model
{
    ...
    final protected function setInfo($data = [], $where = [], $sequence = null)
    {
        ...
        if (empty($data[$pk])&&(!array_key_exists($pk,$data)||(array_key_exists($pk,$data)&& $data[$pk]!=="0"))) {
        //新增
            $update =false;
            if ( !empty($where)||!empty($data[$pk]))
            {
                $update=true;
            }
            $return_data = $this->allowField(true)->isUpdate($update)->save($data, $where, $sequence);
...
//        $return_data = $this->allowField(true)->where($where)->update($data);
    ...
    final protected function setList($data_list = [], $replace = false)
    {
        ...
        $return_data = $this->allowField(true)->saveAll($data_list, $replace);
     ...
     * 获取某个列的数组
     */
    final protected function getColumn($where = [], $field = '', $key = '',$limit=null,$order='')
    {
        
        return Db::name($this->name)->where($where)->order($order)->limit($limit)->column($field, $key);
    }
