...
//        defined('CONTROLLER_NAME')  or define('CONTROLLER_NAME', strtolower($this->request->controller()));
        defined('CONTROLLER_NAME')  or define('CONTROLLER_NAME', humpToLine($this->request->controller()));
    ...
    /**
     * 系统通用跳转
     * @param null $jump_type RESULT_SUCCESS|RESULT_ERROR|RESULT_REDIRECT
     * @param null $message
     * @param null $url
     * @throws \Exception
     */
    final protected function jump($jump_type = null, $message = null, $url = null)
    {
        ...
        {
            case $success  : $this->$success($data[$m], $data[$u],json_encode($data[$info])); break;
    ...
    final protected function  parseJumpArray($jump_array = [])
    {
        ...
        return ['jump_type' => $jump_array[0], 'message' => $jump_array[1], 'url' => $jump_array[2],'info'=>$jump_array[3]];
    }
