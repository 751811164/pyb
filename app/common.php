...
function is_login()
{
    ...
    if (empty($admin)) {
        ...
        return session('admin_auth_sign') == data_auth_sign($admin) ? $admin['admin_id'] : DATA_DISABLE;
    }
}
...
function is_administrator($admin_id = null)
{
    ...
    $return_id = is_null($admin_id) ? is_login() : $admin_id;
...
/**
 * 字符串替换
 * @param $content 被搜索的字符串$content
 * @param $find 要查找的值$find
 * @param $replace 替换$target的内容$replace
 */
function sr($content = '', $find = '', $replace = '')
{
 ...
 * 获取图片url
 */
function get_picture_url($id = 0,$thumb='')
{

    return action(SYS_COMMON_DIR_NAME . SYS_DS_PROS . 'FileBase' . SYS_DS_PROS . 'getPictureUrl', ['id' => $id,'thumb'=>$thumb]);
}
 ...
 * @return string 完整的时间显示
 */
function format_strtime($strtime = '', $format='Y-m-d H:i:s')
{
...
/**
 * Summary of array_sort
 * @param array $list 
 * @param string $key 
 * @param mixed $sort 
 * @return array|boolean
 */
function array_sort(array $list=[], $key, $sort=1){
    if(count($list)==count($list,1)){return false;}//判断是否二维数组

    foreach ($list as $k => $v) {
        $arr[$k] = $v[$key];
    }

    array_multisort($arr,$sort?SORT_ASC:SORT_DESC,$list);//$list按key值排序
    return $list;
