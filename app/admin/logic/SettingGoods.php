...
class SettingGoods extends AdminBase
{
    ...
    {
        $info = $this->modelSettingGoods->getInfo($where, $field);
        return $info;
    }
    ...
    public function getSettingList($where = [], $field = 's.name shop_name,g.name group_name,ss.*', $order = 'ss.status desc,ss.id desc', $paginate = DB_LIST_ROWS)
    {
        ...
        $this->modelSettingGoods->alias('ss');
...
//        $where['g.'.DATA_STATUS_NAME] = ['neq', DATA_DELETE];
         ...
         $info= $this->modelSettingGoods->getList($where, $field, $order, $paginate, $join);
        return $info;
    }

    public function getSimpleSettingList($where=[],$field=true,$order,$paginate = DB_LIST_ROWS){
        return $this->modelSettingGoods->getList($where, $field, $order, $paginate);
    }
    ...
    public function SettingEdit($data=[]){
        ...
        $validate_result = $this->validateSettingGoods->scene($data['mode']==1?'fixEdit':'reachEdit')->check($data);
        if (!$validate_result) : return [RESULT_ERROR, $this->validateSettingGoods->getError()]; endif;
        ...
        $result = $this->modelSettingGoods->setInfo($data);

        $handle_text = empty($data['id']) ? '新增' : '编辑';

        $result && action_log($handle_text, '类别提成规则' . $handle_text . '，group：' . $data['group_id']);

        return $result ? [RESULT_SUCCESS, '操作成功'] : [RESULT_ERROR, $this->modelSettingGoods->getError()];
