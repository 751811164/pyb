...
class Statistic extends AdminBase
{
     ...
     * 会员增长
     */
    public function adminGrowth()
    {
        
        $cache_data = cache('cache_admin_growth_data');
        ...
        $data = $this->getAdminGrowthStruct();

        $list = $this->getAdminGrowthData($data);

        $result = $this->formatAdminGrowthData($data, $list);
        
        // 缓存一天
        cache('cache_admin_growth_data', $result, 86400);
     ...
     * 格式化会员统计数据
     */
    public function formatAdminGrowthData($data, $list)
    {
     ...
     * 获取会员增长数据15天数据
     */
    public function getAdminGrowthData($data)
    {
        ...
        $list = $this->modelAdmin->getList($where, 'id,create_time,is_inside,status', '', false);
     ...
     * 获取会员增长数据15天结构
     */
    public function getAdminGrowthStruct()
    {
    ...
    public function getAdminTree()
    {
        ...
        $cache_data = cache('cache_admin_tree_data');
        ...
        $list = $this->modelAdmin->getList(['status' => DATA_NORMAL, 'is_inside' => DATA_NORMAL], 'id,username,status,leader_id,is_inside', '', false);
        
        $list_tree = list_to_tree($list, 'id', 'leader_id', 'children', DATA_DISABLE, ['username' => 'name', 'leader_id' => 'value']);
        
        $data = $this->compositionAdminTreeData($list_tree);
        
        // 缓存一天
        cache('cache_admin_tree_data', $data[0], 86400);
     ...
     * 递归组装权限等级统计数据
     */
    public function compositionAdminTreeData($list_tree = [])
    {
        ...
        foreach ($list_tree as $k => $v)
        {
            ...
            !empty($v[$kk]) && $data[$k][$kk] = $this->compositionAdminTreeData($v[$kk]);
        }
