...
class SettingOther extends AdminBase
{
    ...
    {
        $info = $this->modelSettingOther->getInfo($where, $field);
        return $info;
    }
    ...
    public function getSettingList($where = [], $field = 'ss.*,username', $order = 'ss.status desc,ss.id desc', $paginate = DB_LIST_ROWS)
    {
        ...
        $this->modelSettingOther->alias('ss');
...
//            [SYS_DB_PREFIX.'auth_group g', 'g.id = ss.group_id', 'LEFT'],
        ...
        $list= $this->modelSettingOther->getList($where, $field, $order, $paginate, $join);
        return  $list;
    }

    public function getSimpleSettingList($where=[],$field=true,$order='',$paginate = DB_LIST_ROWS){
        return $this->modelSettingOther->getList($where, $field, $order, $paginate);
    }
    ...
    public function SettingEdit($data=[]){
        ...
        $validate_result = $this->validateSettingOther->scene('edit')->check($data);
        if (!$validate_result) : return [RESULT_ERROR, $this->validateSettingOther->getError()]; endif;
        ...
        $result = $this->modelSettingOther->setInfo($data);

        $handle_text = empty($data['id']) ? '新增' : '编辑';

        $result && action_log($handle_text, '其他奖励规则' . $handle_text . '，group：' . $data['group_id']);

        return $result ? [RESULT_SUCCESS, '操作成功'] : [RESULT_ERROR, $this->modelSettingOther->getError()];
