...
class AdminExpense extends AdminBase
{
    ...
    public function getExpenseList($where = [], $field = 'a.*', $order = '',$pagenate=0,$join=[])
    {
        ...
        $this->modelAdminExpense->alias('a');
        //$join = [
        ...
        return $this->modelAdminExpense->getList($where, $field, $order, $pagenate, $join);
    }
...
//        if (!$validate_result) : return [RESULT_ERROR, $this->validateExpense->getError()]; endif;
        ...
        $result = $this->modelAdminExpense->setInfo($data);
...
//       $list= collection($this->getExpenseList([],true,'id,name',false))->toArray();
        ...
        return $result ? [RESULT_SUCCESS, '操作成功', $url,['id'=>$lastInsID,'name'=>$data['name']]] : [RESULT_ERROR, $this->modelAdminExpense->getError()];
    }
    ...
    public function getExpenseInfo($where = [], $field = true)
    {
        ...
        return $this->modelAdminExpense->getInfo($where, $field);
    }
    ...
    public function expenseDel($where = [])
    {
        ...
        $result = $this->modelAdminExpense->deleteInfo($where);

        $result && action_log('删除', 'POS机删除' . '，where：' . http_build_query($where));

        return $result ? [RESULT_SUCCESS, '删除成功'] : [RESULT_ERROR, $this->modelAdminExpense->getError()];
    }
