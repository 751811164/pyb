...
class AuthGroupAccess extends AdminBase
{
    ...
    /**
     * 获得权限菜单列表
     * @todo  后期可以像th5-auth一样,在menu里添加额外condition条件,然后在此处验证condition
     */
    public function getAuthMenuList($admin_id = 0)
    {
        ...
        // 获取用户组列表
        $group_list = $this->getAdminGroupInfo($admin_id);
     ...
     * 获取会员所属权限组信息
     */
    public function getAdminGroupInfo($admin_id = 0)
    {
        
        $this->modelAuthGroupAccess->alias('a');
        
        is_array($admin_id) ? $where['a.admin_id'] = ['in', $admin_id] : $where['a.admin_id'] = $admin_id;
        
        $where['a.status']    = DATA_NORMAL;
        
        $field = 'a.admin_id, a.group_id, g.name, g.describe, g.rules';
