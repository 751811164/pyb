...
class Expense extends AdminBase
{
    ...
    {
        $where = $this->logicExpense->getWhere($this->param);
        $this->assign('list', $this->logicExpense->getExpenseList($where));
    ...
    {
        IS_POST && $this->jump($this->logicExpense->expenseEdit($this->param));
    ...
    {
        $this->ExpenseCommon();
        $info = $this->logicExpense->getExpenseInfo(['id' => $this->param['id']]);
    ...
    public function expenseDel($id = 0)
    {
        ...
        $this->jump($this->logicExpense->expenseDel(['id' => $id]));
    }
