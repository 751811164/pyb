...
class PurchaseBackLog extends AdminBase
{
    ...
    public function logList()
    {
        ...
        if (!empty($this->param["back_id"]))
        {


            //采购收货单信息
            $info=$this->logicPurchaseBack->getBillJoinInfo(["pb.id"=>$this->param["back_id"]],"pb.*,add.username admin_add_name,check.username admin_check_name,pi.sn in_sn");

            $where = $this->logicPurchaseBackLog->getWhere($this->param);
            //列表明细
            $list=$this->logicPurchaseBackLog->getLogList($where,"pl.*,gf.barcode1,gf.name,gf.unit,gf.specification",'pl.id asc',false);
            // 入库单信息
//            $billInfo=$this->logicPurchaseBill->getBillInfo(["id"=>$info["bill_id"],DATA_STATUS_NAME=>DATA_NORMAL],true);
    ...
    public function billChoose(){
        $this->view->engine->layout(false);
        $where["pi.".DATA_STATUS_NAME] =DATA_NORMAL;
        $where["pi.use_status"] =DATA_DISABLE;
        $list=  $this->logicPurchaseIn->getBillList($where);
        $this->assign('list', $list);
     ...
     * 获取入库单明细
     */
    public function getPurchaseInLogList(){
        $list=$this->logicPurchaseInLog->getLogList(
            ["in_id"=>$this->param["in_id"],"pl.".DATA_STATUS_NAME=>DATA_NORMAL],
    ...
    public function existBarcode(){
         $this->jump($this->logicPurchaseBackLog->existBarcode($this->param));
    }
    ...
    public function logEdit()
    {
        ...
        IS_POST && $this->jump($this->logicPurchaseBackLog->logEdit($this->param));
...
//        $this->assign("shopList",$shopList=$this->logicShop->getSimpleShopList([],true,'',false));
    ...
    public function checking(){
        $this->jump($this->logicPurchaseBackLog->checking( $this->param));
    }
    ...
    public function unchecking(){
        $this->jump($this->logicPurchaseBackLog->unchecking( $this->param));
    }
    ...
    {
        //审核过的商品不能删除
        $this->jump($this->logicPurchaseBackLog->logDel(['id' => ["in",trim($this->param["ids"] ,',')],'back_id'=>$this->param["back_id"] ]));
    }
    ...
    public function exportLogList()
    {
        ...
        $this->logicPurchaseBackLog->exportLogList($where);
    }
