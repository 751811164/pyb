...
class AdminBase extends ControllerBase
{
    ...
    final private function initAdminInfo()
    {
        ...
        // 验证登录
        !ADMIN_ID && $this->redirect('login/login');

        // 获取授权菜单列表
        $this->authMenuList = $this->logicAuthGroupAccess->getAuthMenuList(ADMIN_ID);

        // 获得权限菜单URL列表
        $this->authMenuUrlList = $this->logicAuthGroupAccess->getAuthMenuUrlList($this->authMenuList);
        
        // 检查菜单权限
        list($jump_type, $message) = $this->logicAdminBase->authCheck($this->request->path(), $this->authMenuUrlList);
    ...
    final private function initAdminConst()
    {
        ...
        // 会员ID
        defined('ADMIN_ID')    or  define('ADMIN_ID',     is_login());
        
        // 是否为超级管理员
        defined('IS_ROOT')      or  define('IS_ROOT',       is_administrator());

        //
        defined('SHOP_ID')      or  define('SHOP_ID',       config('shop_id'));
        defined('HEAD_OFFICE')      or  define('HEAD_OFFICE',       config('head_office'));
    }
