...
class SimpleCalendar {
  ...
  create() {
    ...
    header.innerHTML = header.innerHTML + '<div class="sc-actions"><select class="sc-select-multiday" style="border-color: #F1EBE4;width: 80px;"><option value="0">全选</option><option value="1">单数</option> <option value="2">双数</option>  </select></div>';

    header.innerHTML = header.innerHTML + '<div class="sc-actions" hidden><span class="sc-time"></span></div>';
  ...
  addData(year, month) {
    ...
    for (var i = 0; i < 42; i++) {
      ...
      daysElement[i].setAttribute("data-day",writeday);
      //判断是否添加阴历
  ...
  updateEvent() {
    ...
    daysElement.forEach(function (v, i) {
      ...
      v.onclick = function () {
        if (this.classList.contains("sc-othermenth")) {
          return false;
        }
    ...
    selectYear.onchange = function (e) {
      ...
      var y = this.value;
      calendar.update(m, y);
      if(typeof calendar._options.afterChange==='function'){
        typeof calendar._options.afterChange(e,calendar,selectYear.value,selectMonth.value);
      }
    };
    ...
    returntoday.onclick = function (e) {
    ...
    //批量选择 全选/单数/双数
    var multiday = container.querySelector(".sc-select-multiday");
    multiday.onchange=function(e){
      var items = container.querySelectorAll('.sc-item:not(.sc-othermenth)');
      for (var i = 0; i < items.length; i++) {
        ...
        if (e.currentTarget.value==0){
          ...
          item.classList.remove('sc-selected');

          if (e.currentTarget.value ==1 && i %2==0){
            //单数
            console.log("1");
            item.classList.add('sc-selected');
          }
          if(e.currentTarget.value ==2 && i %2) {
            //双数
            console.log("2");
            item.classList.add('sc-selected');
          }
