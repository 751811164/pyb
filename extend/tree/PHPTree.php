...
class PHPTree
{
     ...
     * @return mixed 多维数组
     */
    public static function makeTree($data, $options = array(), $closure = false)
    {
        $dataset = self::buildData($data, $options);
        $r       = empty($data)?[]:self::makeTreeCore(0, $dataset, 'normal', $closure);
        return $r;
    }
    ...
    /* 生成树核心, 私有方法  */
    private static function makeTreeCore($index, $data, $type = 'linear', $closure = false)
    {
        extract(self::$config);
        foreach ($data[$index] as $id => $item)
        {
            if ($type == 'normal')
            {
                ...
                if (isset($data[$id]))
                {
                    ...
                    $item[$children_key] = self::makeTreeCore($id, $data, $type, $closure);
                }
                else
                {
                    if ($item['level']==config('logic_config.category_level'))
                    {
                        $item[$leaf_key] = true;
                    }
                    else
